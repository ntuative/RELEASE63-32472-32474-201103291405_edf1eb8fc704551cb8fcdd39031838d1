package com.sulake.habbo.widget.roomqueue
{
   ...
   public class RoomQueueWidget extends RoomWidgetBase
   {
      ...
      override public function registerUpdateEvents(param1:IEventDispatcher) : void
      {
         ...
         param1.addEventListener(RoomWidgetRoomQueueUpdateEvent.const_368,this.onQueueStatus);
      ...
      override public function unregisterUpdateEvents(param1:IEventDispatcher) : void
      {
         ...
         param1.removeEventListener(RoomWidgetRoomQueueUpdateEvent.const_368,this.onQueueStatus);
      ...
      private function onQueueStatus(param1:RoomWidgetRoomQueueUpdateEvent) : void
      {
         ...
         if(param1.isActive)
         {
            this.var_1747 = param1.type;
            this.var_1395 = false;
            this.var_1135 = param1.position;
         }
         else
         {
            this.var_1395 = true;
         }
         this.var_2378 = param1.hasHabboClub;
         this.var_1746 = param1.isClubQueue;
         localizations.registerParameter("room.queue.position","position",this.var_1135.toString());
         localizations.registerParameter("room.queue.position.hc","position",this.var_1135.toString());
         localizations.registerParameter("room.queue.spectator.position","position",this.var_1135.toString());
         localizations.registerParameter("room.queue.spectator.position.hc","position",this.var_1135.toString());
         this.showInterface();
      }
      ...
      private function showInterface() : void
      {
         ...
         if(_loc1_ != null && _loc2_ != null && _loc3_ != null)
         {
            ...
            {
               case RoomWidgetRoomQueueUpdateEvent.const_368:
                  _loc1_.caption = !!this.var_1746 ? "${room.queue.position.hc}" : "${room.queue.position}";
                  _loc2_.caption = "${room.queue.spectatormode}";
                  _loc3_.visible = this.var_1395;
                  break;
               case RoomWidgetRoomQueueUpdateEvent.const_581:
                  _loc1_.caption = !!this.var_1746 ? "${room.queue.spectator.position.hc}" : "${room.queue.spectator.position}";
      ...
      private function exitQueue(param1:WindowMouseEvent) : void
      {
         ...
         var _loc2_:RoomWidgetRoomQueueMessage = new RoomWidgetRoomQueueMessage(RoomWidgetRoomQueueMessage.const_770);
      ...
      private function changeQueue(param1:WindowMouseEvent) : void
      {
         ...
         if(this.var_1747 == RoomWidgetRoomQueueUpdateEvent.const_368)
         {
            _loc2_ = new RoomWidgetRoomQueueMessage(RoomWidgetRoomQueueMessage.CHANGE_TO_SPECTATOR_QUEUE);
         }
         else
         {
            _loc2_ = new RoomWidgetRoomQueueMessage(RoomWidgetRoomQueueMessage.const_705);
         }
         messageListener.processWidgetMessage(_loc2_);
         this.removeWindow();
      }
