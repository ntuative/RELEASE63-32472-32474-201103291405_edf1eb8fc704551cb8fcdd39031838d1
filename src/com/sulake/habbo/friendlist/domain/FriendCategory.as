package com.sulake.habbo.friendlist.domain
{
   ...
   public class FriendCategory implements IDisposable
   {
      ...
      {
         if(this.var_996 >= this.getPageCount())
         {
            this.var_996 = Math.max(0,this.getPageCount() - 1);
         }
      }
      
      public function getPageCount() : int
      {
         return Math.ceil(this.var_229.length / const_373);
      }
      
      public function getStartFriendIndex() : int
      {
         this.checkPageIndex();
         return this.var_996 * const_373;
      }
      
      public function getEndFriendIndex() : int
      {
         this.checkPageIndex();
         return Math.min((this.var_996 + 1) * const_373,this.var_229.length);
      }
