package com.sulake.habbo.avatar
{
   ...
   public class AvatarAssetDownloadManager extends EventDispatcher
   {
      ...
      private var var_941:int = 3;
      
      private var var_434:AssetLoaderStruct;
      
      private var var_943:Timer;
      ...
      private const const_2064:int = 100;
      ...
      public function AvatarAssetDownloadManager(param1:IAssetLibrary, param2:String, param3:String, param4:AvatarStructure)
      {
         ...
         this.var_943 = new Timer(this.const_2064,1);
         this.var_943.addEventListener(TimerEvent.TIMER_COMPLETE,this.onNextDownloadTimeout);
      }
      ...
      private function addMapLoaderEventListeners() : void
      {
         if(this.var_434)
         {
            ...
            this.var_434.addEventListener(AssetLoaderEvent.const_49,this.onConfigurationError);
         }
      }
      ...
      private function removeMapLoaderEventListeners() : void
      {
         if(this.var_434)
         {
            ...
            this.var_434.removeEventListener(AssetLoaderEvent.const_49,this.onConfigurationError);
         }
      }
      ...
      private function onConfigurationError(param1:Event) : void
      {
         ...
         {
            Logger.log("Could not download figuremap.xml. Retries left: " + this.var_941);
            if(this.var_1414.indexOf("?") > 0)
            {
               _loc2_ = this.var_1414 + "&retry=" + this.var_941;
            }
            else
            {
               _loc2_ = this.var_1414 + "?retry=" + this.var_941;
      ...
      private function generateMap(param1:XML) : void
      {
         ...
         {
            _loc3_ = new AvatarAssetDownloadLibrary(_loc2_.@id,_loc2_.@revision,this.var_2454,this._assets);
      ...
      public function loadFigureSetData(param1:IAvatarFigureContainer, param2:IAvatarImageListener) : void
      {
         ...
         if(!this.var_1784)
         {
            this.var_1166.push([param1,param2]);
            return;
         }
         ...
         if(_loc4_.length > 0)
         {
            ...
            {
               _loc6_ = this.var_554[_loc3_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(param2);
               this.var_554[_loc3_] = _loc6_;
            }
      ...
      private function libraryComplete(param1:Event) : void
      {
         ...
         for(_loc3_ in this.var_942)
         {
            ...
            if(_loc7_)
            {
               ...
               delete this.var_554[_loc3_];
            }
         }
         for each(_loc3_ in _loc2_)
         {
            delete this.var_942[_loc3_];
         }
         while(_loc6_ < this.var_751.length)
         {
            _loc5_ = this.var_751[_loc6_];
            if(_loc5_.libraryName == (param1.target as AvatarAssetDownloadLibrary).libraryName)
            {
               this.var_751.splice(_loc6_,1);
            }
            _loc6_++;
         }
         this.var_943.start();
      ...
      {
         var _loc1_:* = null;
         while(this.var_750.length > 0 && this.var_751.length < this.const_1682)
         {
            _loc1_ = this.var_750[0];
            _loc1_.startDownloading();
            this.var_751.push(this.var_750.shift());
         }
      }
      
      private function addToQueue(param1:AvatarAssetDownloadLibrary) : void
      {
         if(!param1.isReady && this.var_750.indexOf(param1) == -1 && this.var_751.indexOf(param1) == -1)
         {
