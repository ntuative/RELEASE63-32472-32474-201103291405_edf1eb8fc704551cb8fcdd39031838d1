package com.sulake.habbo.communication
{
   ...
   public class HabboCommunicationManager extends Component implements IHabboCommunicationManager, IConnectionStateListener
   {
      ...
      private const const_2044:int = 2;
      ...
      private var var_1509:IMessageConfiguration;
      ...
      {
         this.var_1509 = new HabboMessages();
         this.var_245 = [];
      ...
      {
         if(this.var_245.length == 0 || this.var_408 < 0 || this.var_408 >= this.var_245.length)
         {
            return 0;
         }
         return this.var_245[this.var_408];
      }
      ...
      private function onCoreCommunicationManagerInit(param1:IID = null, param2:IUnknown = null) : void
      {
         ...
         if(param2 != null)
         {
            ...
            this.var_290.registerProtocolType(HabboProtocolType.const_1175,WedgieProtocol);
            this._connection = this.var_290.createConnection(HabboConnectionType.const_232,ConnectionType.const_1266);
            _loc3_ = this.var_290.getProtocolInstanceOfType(HabboProtocolType.const_1175);
            this._connection.registerMessageClasses(this.var_1509);
      ...
      private function onHabboConfigurationInit(param1:IID = null, param2:IUnknown = null) : void
      {
         ...
         if(param2 != null)
         {
            ...
            this.var_245 = [];
            this.var_37 = this.var_164.getKey("connection.info.host",null);
            if(this.var_37 == null)
            {
               Core.crash("connection.info.host was not defined",Core.const_318);
               return;
            }
            _loc3_ = this.var_164.getKey("connection.info.port",null);
            if(_loc3_ == null)
            {
               Core.crash("connection.info.port was not defined",Core.const_318);
               return;
            }
            if(this.var_164.keyExists("local.environment") && this.var_164.getKey("local.environment") == "true")
            {
               this.var_37 = this.var_164.getKey("connection.info.host.local");
               _loc3_ = this.var_164.getKey("connection.info.port.local");
            }
            _loc4_ = this.validateHost();
            if(!_loc4_)
            {
               Core.crash("Tried to connect to an invalid host: " + this.var_37,Core.const_318);
               return;
            }
            _loc5_ = _loc3_.split(",");
      ...
      public function initConnection(param1:String) : void
      {
         switch(param1)
         {
            case HabboConnectionType.const_232:
               ...
               {
                  Core.crash("Tried to connect to proxy but configuration was null",Core.const_318);
                  return;
               }
               if(this._connection == null)
               {
                  Core.crash("Tried to connect to proxy but connection was null",Core.const_318);
                  return;
               }
               this.var_2194 = true;
               if(this.var_2193)
      ...
      private function nextPort() : void
      {
         ...
         ++this.var_408;
         if(this.var_408 >= this.var_245.length)
         {
            ErrorReportStorage.addDebugData("ConnectionRetry","Connection attempt " + this.var_878);
            ++this.var_878;
            _loc1_ = this.const_2044;
            if(this.var_245.length == 1)
            {
               _loc1_++;
            }
            if(this.var_878 > _loc1_)
            {
               if(this.var_2192)
               {
                  return;
               ...
               this.var_2192 = true;
               Core.error("Connection failed to host " + this.var_37 + " ports " + this.var_245,true,Core.const_318);
               return;
            }
            this.var_408 = 0;
         }
         this._connection.timeout = this.var_878 * 10000;
         this._connection.init(this.var_37,this.var_245[this.var_408]);
      }
      ...
      private function onIOError(param1:IOErrorEvent) : void
      {
         ...
         ErrorReportStorage.addDebugData("Communication IO Error","IOError " + param1.type + " on connect: " + param1.text + ". Port was " + this.var_245[this.var_408]);
         this.tryNextPort();
      }
      
      private function onConnect(param1:Event) : void
      {
         ErrorReportStorage.addDebugData("Connection","Connected with " + this.var_878 + " attempts");
      }
      ...
      private function onSecurityError(param1:SecurityErrorEvent) : void
      {
         ...
         ErrorReportStorage.addDebugData("Communication Security Error","SecurityError on connect: " + param1.text + ". Port was " + this.var_245[this.var_408]);
         this.tryNextPort();
      }
      
      public function getHabboMainConnection(param1:Function) : IConnection
      {
         return !!this.var_290 ? this.var_290.queueConnection(HabboConnectionType.const_232,param1) : null;
      }
      ...
      public function addHabboConnectionMessageEvent(param1:IMessageEvent) : void
      {
         if(this.var_290)
         {
            this.var_290.addConnectionMessageEvent(HabboConnectionType.const_232,param1);
         }
      }
      ...
      {
         ErrorReportStorage.setParameter(HabboErrorVariableEnum.const_1652,param1);
         ErrorReportStorage.setParameter(HabboErrorVariableEnum.const_1638,String(param2));
      }
      
      public function messageReceived(param1:String, param2:String) : void
      {
         ErrorReportStorage.setParameter(HabboErrorVariableEnum.const_1582,String(new Date().getTime()));
      ...
      {
         ErrorReportStorage.setParameter(HabboErrorVariableEnum.const_1520,String(new Date().getTime()));
