package com.sulake.core.window.utils
{
   ...
   public class MouseEventProcessor implements IEventProcessor, IDisposable
   {
      ...
      public function MouseEventProcessor()
      {
         ...
         {
            var_336 = new Array();
            var_336[0] = MouseCursorType.const_305;
            var_336[1] = MouseCursorType.const_32;
            var_336[2] = MouseCursorType.const_305;
            var_336[3] = MouseCursorType.const_305;
            var_336[4] = MouseCursorType.const_305;
            var_336[5] = MouseCursorType.const_32;
            var_336[6] = MouseCursorType.const_305;
         }
         if(var_288 == null)
         {
            var_288 = new Array();
            var_288[0] = WindowState.const_105;
            ...
            var_288[6] = WindowState.const_83;
         }
      }
      ...
      protected static function convertMouseEventType(param1:MouseEvent, param2:IWindow, param3:IWindow) : WindowMouseEvent
      {
         ...
         switch(param1.type)
         {
            ...
            case MouseEvent.MOUSE_UP:
               ...
               _loc4_ = !!_loc6_ ? "null" : WindowMouseEvent.const_198;
               break;
      ...
      public function process(param1:EventProcessorState, param2:IEventQueue) : void
      {
         ...
         this.var_74 = state.var_1298 as WindowController;
         this.var_177 = state.var_1295 as WindowController;
         this.var_170 = state.renderer;
         this.var_858 = state.var_1299;
         eventQueue.begin();
         this.var_692.x = -1;
         this.var_692.y = -1;
         ...
         while(true)
         {
            ...
            if(event.stageX != this.var_692.x || event.stageY != this.var_692.y)
            {
               this.var_692.x = event.stageX;
               this.var_692.y = event.stageY;
               array = new Array();
               this.var_146.groupParameterFilteredChildrenUnderPoint(this.var_692,array,WindowParam.const_30);
            }
            ...
            if(index == 0)
            {
               if(event.type == MouseEvent.MOUSE_MOVE)
               {
                  ...
                  {
                     this.var_74.getGlobalPosition(var_404);
               ...
               if(child != null && child.visible)
               {
                  ...
                  else if(event.type == MouseEvent.MOUSE_UP || event.type == MouseEvent.CLICK)
                  {
                     if(this.var_74 && !this.var_74.disposed)
                     {
                        ...
                        {
                           this.var_858.mouseEventReceived(event.type,this.var_74);
                        }
                     }
                  }
                  temp = child.parent;
         ...
         state.var_1298 = this.var_74;
         state.var_1295 = this.var_177;
         state.renderer = this.var_170;
         state.var_1299 = this.var_858;
      }
