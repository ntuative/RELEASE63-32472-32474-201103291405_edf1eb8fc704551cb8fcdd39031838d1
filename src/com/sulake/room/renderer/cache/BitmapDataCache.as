package com.sulake.room.renderer.cache
{
   ...
   public class BitmapDataCache
   {
      ...
      public function compress() : void
      {
         ...
         {
            this.var_452.sortOn("useCount",0 | 0);
            _loc2_ = this.var_452.length - 1;
            while(_loc2_ >= 0)
            {
               _loc1_ = this.var_452[_loc2_] as BitmapDataCacheItem;
            ...
            this.var_452.splice(_loc2_,this.var_452.length - _loc2_);
            if(this.memUsage > this.memLimit)
            {
               this.var_452.sortOn("memUsage",Array.NUMERIC);
               while(this.memUsage > this.memLimit)
               {
                  _loc1_ = this.var_452.pop() as BitmapDataCacheItem;
                  if(_loc1_ == null)
                  {
                     break;
                  }
